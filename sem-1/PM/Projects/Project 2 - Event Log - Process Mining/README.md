...
Among end activities, in 393 cases the last activity is **"Release A"**, in 93 cases other _'releases'_, in 291 cases it is **"Return ER"**, in total 777 cases (74% of all traces) end with patient discharge.
...
- Classical algorithms like _alpha_ explicitly don't support short loops.
...
_Since anonymization was applied to the dataset (while intra-case time relations, such as time differences between activities, were preserved), **the inter-case relations (such as monthly or hourly distributions) cannot be analysed**_. 
...
<div style="width:50%; margin: auto;" >
   ...
   ![Monthly Case Frequency](images/monthly_case_frequency.png)   
</div>
...
![alt text](images/case_duration_distribution.png)
...
## Experiment 1. Preprocessing: Noise & Relationship Filtering
...
$\rightarrow ( +( \circlearrowright ( \text{'Leucocytes'}, \tau ), \rightarrow( +( \circlearrowright( \text{'CRP'}, \tau ), \circlearrowright( \text{'LacticAcid'}, \tau ), \times( \tau, \text{'IV Liquid'} ), $
$\ \ \ \ \ \ \ \ \ \ \ \ \rightarrow( \times( \tau, +( \text{'ER Triage'}, \rightarrow( \text{'ER Sepsis Triage'}, \text{'IV Antibiotics'} ), \text{'ER Registration'} ), \circlearrowright( \text{'Admission NC'}, \tau ) ) ), \text{'Release A'} ) ),$
$\ \ \ \ \ \ \times( \tau, \text{'Release B'}, \rightarrow( \times( \tau, \text{'Release D'}, \text{'Release E'}, \text{'Release C'} ), \text{'Return ER'} ) ) )$

Or schematically: 

<div align = center width="70%">
<img src="images/best_process_tree.png" width="70%">
</div>
...
<div align = center >
<img src="images/best_cnet.png" width="85%">
...
- **Classical Soundness Check**: `pm4py.algo.analysis.woflan.algorithm.apply = False`.
...
<div align = center >
<img src="images/best_process_tree_exp2.png" width="70%">
</div>
...
<div align = center >
<img src="images/best_cnet_exp2.png" width="50%">
</div>
...
- **C-net**:
  - **Classical Petri Net Soundness**: Formally **`False`** under strict Woflan verification, but **substantially improved compared to Experiment 1**.
  ...
  - **Tokens Left Inside the Model & Clean Trace Completions**:
    ...
    - In Experiment 2, the model now successfully achieves **clean trace completions** without missing or stranded tokens on dominant standard pathways ($ \text{ER Registration} \to \text{ER Triage} \to \text{ER Sepsis Triage} \to \text{IV Liquid} \parallel \text{IV Antibiotics} \to \text{Admission NC} \to \text{Release C/D}$).
...
- $\text{Leucocytes} \to \text{CRP}$ (223 occurrences)
- $\text{Leucocytes} \to \text{CRP} \to \text{LacticAcid}$ (173 occurrences)
- $\text{CRP} \to \text{Leucocytes}$ (156 occurrences)
- $\text{LacticAcid} \to \text{Leucocytes} \to \text{CRP}$ (131 occurrences)
- Multi-draw alternating cycles: $\text{LCLC}$ (33), $\text{LCCL}$ (33), $\text{CLLC}$ (28), $\text{CLCL}$ (25)...
...
<div align=center width="70%">
<img src="images/best_process_tree_exp3.png" width="70%">
</div>
...
<div align=center width="70%">
<img src="images/best_cnet_exp3.png" width="70%">
</div>

<div style="page-break-after: always;"></div>

### Soundness Verification (Experiment 3)
- **Process Tree - sound WF-Net by construction**.
...
Under this model, the dominant clinical care pathway matches previously manually identified exploratory trace (`A,C,A,B,A`):
...
#### Consistency with Manual Analysis
  - **Dominant prefix** confirmed by the standard intake protocol ($\text{ER Registration} \to \text{ER Triage} \to \text{ER Sepsis Triage}$) observed in 73.1% of patients.
...
| **Sepsis Triage $\to$ IV Antibiotics** | **Critical Hour - Key Benchmark** | **92.2 min** | **166.1 min** | **Operational Bottleneck**: 75% of patients exceed the [60-min Surviving Sepsis guideline](https://doi.org/10.1097/CCM.0000000000005337). |
...
![alt text](images/disco_typical_process_flow.png)
...
- **Intake Conformity**: The manual prefix analysis revealed that **73.1% of all cases (768/1,050)** strictly start with the triage sequence $ \text{ER Registration} \to \text{ER Triage} \to \text{ER Sepsis Triage}$. The process map confirms this as the heaviest pipeline in the hospital (weights 971 and 905).
...
![alt text](images/disco_activity_frequencies.png)
...
![alt text](images/disco_loops.png)
...
1. **Dominant Intake Backbone**: Both models confirm that over **73% of patients** follow the strict linear triage sequence $\text{ER Registration} \to \text{ER Triage} \to \text{ER Sepsis Triage}$.
